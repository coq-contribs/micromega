...
struct
 ...
 let gcd  t =Utils.gcd_list t




 let hash = Utils.Cmp.hash_list int_of_num 

 let compare = Utils.Cmp.compare_list compare_num
...
struct
 ...
 let gcd m = 
  let res = Map.fold (fun _ e x -> Big_int.gcd_big_int  x (Utils.numerator e)) m Big_int.zero_big_int in
...
struct
 ...
 let hash = Utils.Cmp.hash_list (fun (x,y) -> (Hashtbl.hash x) lxor (int_of_num y))


 let compare = Utils.Cmp.compare_list (fun x y -> Utils.Cmp.compare_lexical 
  [
 ...
 (* 
    ...
    | c -> c
    and xcomp acc1 acc2 = Utils.Cmp.compare_list compare_num acc1 acc2 in
    xcompare [] [] x y
 *)
