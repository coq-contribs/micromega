...
(* let coq_n = lazy (constant "N")*)
 ...
 let coq_C = lazy (constant"PEc")
 let coq_Add = lazy (constant "PEadd")
 let coq_UMinus = lazy (constant "PEopp")
 let coq_Mult = lazy (constant "PEmul")
 let coq_Sub = lazy (constant "PEsub")
 let coq_pow = lazy (constant "PEpow")
 ...
 let parse_formula parse_atom env term = 
  ...
  and xparse_formula env term = 
   match kind_of_term term with
	 ...
	 | Const x -> 
	    (
		 ...
		 let (f2,env) = parse_catch env rst.(1) in
		  (match f1 , f2 with
		   |  X _  , X _ -> (X(term),env)
		   |   _         -> (C(I(f1,f2,none),I(f2,f2,none),none),env)
		  )
	      |   _  -> parse_atom env term
	    )
	 | _ -> parse_atom env term
    ...
    | Prod(typ,l,r) -> 
       if is_prop typ then
	let (f1,env) = parse_catch env l in
	let (f2,env) = parse_catch env r in
	 (match f1 , f2 with
	  |  X _  , X _ -> (X(term),env)
	  |   _         -> (I(f1,f2,none),env)
	 )
       else failwith "xparse_formula(2)"
...
let micromega_order_change spec cert cert_typ env  ff gl = 
 ...
 let vm  = dump_varmap  ( spec.typ)  env in
  if debug then (Pp.pp (Printer.prterm  vm); Pp.pp_flush ());
